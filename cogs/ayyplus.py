        ...
        if message.server.id == '133049272517001216': #red's server
            ...
            if message.author.id != self.bot.user.id:
                if "ayy" in message.content.split():
                    ...
                    if enabled:
                        await self.bot.send_message(message.channel, ":ok_hand:")
                elif "boom" in message.content.split():
                    if enabled:
                        await self.bot.send_message(message.channel, "http://www.reactiongifs.com/r/perma_banned.gif")
                elif "TRIGGERED" in message.content.split():
                    if enabled:
                        await self.bot.send_message(message.channel, ":imp:")
                elif "duck" in message.content.split():
                    if enabled:
                        await self.bot.send_message(message.channel, "http://giphy.com/gifs/no-5T8tEJtCgvDuo")						
def check_folders():
