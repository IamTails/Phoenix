...
class Massmove:
    ...
    async def massmovey(self, ctx, from_channel: discord.Channel, to_channel: discord.Channel):
        """Massmove users to another voice channel"""
    ...
    @commands.command(pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def massTM(self, ctx):
        """yup"""
        await self.bot.say("Adding everyone to role 'Team Mystic', please wait! We Win!!!")
        role = discord.utils.get(ctx.message.server.roles, name="Team Mystic")
